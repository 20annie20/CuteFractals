...
public class KochGenerator : MonoBehaviour
{
    ...
    private Vector3[] _initatorPoint;
    ...
    {
        GetInitiatiorPoints();
        _initatorPoint = new Vector3[_initiatorPointAmount];

        _rotateVector = Quaternion.AngleAxis(_initialRotation, _rotateAxis) * _rotateVector;
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            _initatorPoint[i] = _rotateVector * _initiatorSize;
            _rotateVector = Quaternion.AngleAxis(360 / _initiatorPointAmount, _rotateAxis) * _rotateVector;
        }
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            ...
            {
                Gizmos.DrawLine(_initatorPoint[i], _initatorPoint[i + 1]);
            }
            else
            {
                Gizmos.DrawLine(_initatorPoint[i], _initatorPoint[0]);
            }
        }
    }
